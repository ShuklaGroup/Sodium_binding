...
glu_thr_distance = pickle.load(open('./CB2_glu_thr_distance.pkl','rb'))

glu_chi2 = pickle.load(open('./CB2_glu_chi2.pkl','rb'))

#######################################MSM loading##############################################

msm = pickle.load(open("./../CB2_final_MSM_obj.pkl","rb")) #loading of MSM object
...
x_bins = 50     #number of bins used to divide the landscape in x-direction 
y_bins = 50     #number of bins used to divide the landscape in y-direction 
...
x_data =  np.concatenate(glu_thr_distance)*10                     #assign x_data as 1-D array with x-direction feature (change nm to angtrom)
y_data =  np.concatenate(glu_chi2)*180/np.pi                     #assign y_data as 1-D array with y-direction feature (change nm to angtrom)
...
axs.set_xlim([0,14])                                                                          #min and max limit of x-axis
axs.set_ylim([0,360])                                                                            #min and max limit of y-axis

axs.set_xticks(range(int(0),int(14)+1,2))                                                    #x-axis ticks
axs.set_xticklabels(range(int(2),int(14)+1,2))                                               #y-axis ticks

axs.set_yticks(range(int(0),int(360)+1,60))                                                       #x-axis tick size
axs.set_yticklabels(range(int(0),int(360)+1,60))                                                  #y-axis tick size

plt.ylabel('E50$^{1.49} \chi_2$ angle', **hfont,fontsize=30)                                  #y-axis label
plt.xlabel('E50$^{1.49}$-M293$^{7.47}$ distance' + ' (\AA)', **hfont,fontsize=30)              #x-axis label 
